...
get_checksum_for_snapshot_json () {
    ...
    printf -- "- Update recipes-devtool/rust-snapshot-%s.inc:\n\tSRC_URI[%s-snapshot-%s.sha256sum] = " \
        "${RUST_VERSION}" \
        "${RUST_STD_PREFIX}" "${_arch_only}"
    jq .checksums_sha256 <"${OUT}" | grep "$(printf -- "%s-%s-%s.%s" "${RUST_STD_PREFIX}" "${_snapshot_version}" "${_arch}" "${TARBALL_FORMAT}")" |sed -n -r -e 's/\s+".*": "([a-f0-9]{1,64})",$/\1/p'
    printf -- "\n"
...
get_checksum_for_snapshot () {
    ...
    printf -- "- Update recipes-devtool/rust-snapshot-%s.inc:\n\tSRC_URI[%s-snapshot-%s.sha256sum] = " \
        "${RUST_VERSION}" \
        "${RUST_STD_PREFIX}" "${_arch_only}"
    grep "$(printf -- "%s-%s-%s.%s" "${RUST_STD_PREFIX}" "${_snapshot_version}" "${_arch}" "${TARBALL_FORMAT}")" "${OUT}" |cut -d'=' -f2
    printf -- "\n"
